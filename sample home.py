...
# --- Unified Color Theme ---
DARK_PURPLE = "#1a0b36"    # Main background
PRIMARY_DARK = "#1E1E2F"    # Card / frame background
        ...
        with open(filename, 'w') as csvfile:
            ...
            while not thread_event.is_set():
                ...
                if recog_names != old_recognized:
                    # Safely update right_frame only if it still exists
                    if right_frame and right_frame.winfo_exists():
                        ...
                        for i, crim in enumerate(recognized):
                            ...
                            csvwriter.writerow([num, crim[0], x])
                        old_recognized = recog_names

                # Safely show image on main Tkinter thread
                img_size = min(left_frame.winfo_width(), left_frame.winfo_height()) - 20
                if left_frame and left_frame.winfo_exists():
                    root.after(0, lambda f=frame, s=img_size: showImage(f, s))
                else:
                    # Frame was closed — stop loop gracefully
                    thread_event.set()
                    break
...
def on_closing():
    ...
    if thread_event and not thread_event.is_set():
        thread_event.set()
